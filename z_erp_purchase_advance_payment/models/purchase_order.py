...
class PurchaseOrder(models.Model):
    ...
    def _advance_payment_count(self):
        for line in self:
            payment_ids = self.env['account.payment'].search([('purchase_id', '=', line.id)])
    ...
    def action_advance_payment(self):
        view_id = self.env['purchase.advance.payment']
        return {
            'type': 'ir.actions.act_window',
            ...
            'view_mode': 'form',
            'res_id': view_id.id,
            'view_id': self.env.ref('z_erp_purchase_advance_payment.purchase_order_advance_payment_register_wizard', False).id,
            'target': 'new',
...
class purchaseAdvancePayment(models.TransientModel):
    ...
    partner_id = fields.Many2one('res.partner', 'Supplier',required=1)
    ...
    def _onchange_journal(self):
        if self.journal_id:
            ...
            payment_methods = self.payment_type == 'outbound' and self.journal_id.outbound_payment_method_ids or self.journal_id.inbound_payment_method_ids
